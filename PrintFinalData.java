...
public class PrintFinalData extends HttpServlet{
	...
	private void DisplayPage(HttpServletRequest request, HttpServletResponse response)
		...
		/*
		 * Determine the max Y axis length.
		 */
		
		int binWidth = 5;
		
		Map<Integer,Integer> currentHisto = Statistics.buildHisto(statement[i], binWidth);
		
		int maxY = 0;
		for (Integer i : currentHisto.keySet()) 
			if (i > maxY)
				maxY = i;

		int numberOfBins = currentHisto.keySet().size();
		
		int[] barKeys = new int[numberOfBins];
		int[] barValues = new int[numberOfBins];
		
		
		
		for (int i = 0; i <= numberOfBins-1; i++) {
			barKeys[i] = i * binWidth;
			barValues[i] = currentHisto.get(i * binWidth);
		}
		...
		request.setAttribute("Histogram", barValues);
		request.setAttribute("BarGraph", barKeys);
	}
}
